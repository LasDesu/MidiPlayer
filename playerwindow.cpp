...
// STATIC vars
snd_seq_t *PlayerWindow::seq=0;
snd_seq_addr_t *PlayerWindow::ports=0;
double PlayerWindow::song_length_seconds=0;
...
void PlayerWindow::on_PortBox_currentIndexChanged(QString buf)
{
...
//  FUNCTIONS
void PlayerWindow::send_data(char * buf,int data_size) {
	snd_seq_event_t ev;
	snd_seq_ev_clear(&ev);
...
void PlayerWindow::send_SysEx(char * buf,int data_size) {
...
void PlayerWindow::connect_port() {
...
void PlayerWindow::disconnect_port() {
	if (seq && strlen(port_name)) {
		int err;
		ports = (snd_seq_addr_t *)realloc(ports, sizeof(snd_seq_addr_t));
		err = snd_seq_parse_address(seq, &ports[0], port_name);
...
void PlayerWindow::getPorts(QString buf) {
...
void PlayerWindow::getRawDev(QString buf) {
...
void PlayerWindow::startPlayer(int startTick) {
